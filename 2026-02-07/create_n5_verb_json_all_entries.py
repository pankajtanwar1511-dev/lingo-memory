#!/usr/bin/env python3
"""
Generate Complete N5 Verb JSON - ALL 161 ENTRIES
Each row in markdown = separate entry (even if same verb with different meaning)
"""
...
def create_verb_entry(kanji, kana, masu_kanji, masu_kana, meaning, current_group, verb_id):
    """Create a verb entry - ONE ENTRY PER ROW"""
    lookup_key = f"{kanji}|{kana}"
    jmdict_info = jmdict_lookup.get(lookup_key)
    verb_type_info = determine_verb_type(kana, jmdict_info)

    # Extract ALL glosses from JMdict for reference
    all_jmdict_meanings = []
    if jmdict_info:
        for sense in jmdict_info.get('jmdict_verb_senses_only', []):
            all_jmdict_meanings.extend(sense.get('glosses', []))

    # Use the SPECIFIC meaning from this row as PRIMARY
    meaning_clean = re.sub(r'\[.*?\]', '', meaning).strip()

    # Create meaning array: specific meaning first, then all JMdict meanings
    meaning_array = []
    if meaning_clean:
        meaning_array.append(meaning_clean)

    # Add all JMdict meanings that aren't already included
    for jm in all_jmdict_meanings:
        if jm not in meaning_array:
            meaning_array.append(jm)

    # If still empty, use original meaning
    if not meaning_array:
    ...
    return {
        ...
        "meaning": meaning_array,  # ALL meanings with specific one first
        "primaryMeaning": meaning_clean if meaning_clean else meaning,  # The specific meaning for this entry
        "jlptLevel": "N5",
        "partOfSpeech": ["verb"],
...
# Parse verbs from markdown - EVERY ROW = SEPARATE ENTRY
verbs_list = []
...
for i, line in enumerate(lines):
    ...
    # Parse GROUP I and GROUP II verb tables - EVERY ROW COUNTS
    if current_group in ['godan', 'ichidan'] and line.startswith('| '):
        ...
        if len(parts) >= 6 and parts[1] and parts[2]:
            ...
            # CREATE ENTRY FOR THIS ROW (even if duplicate verb)
...
output_json = {
    "metadata": {
        ...
        "description": "Complete JLPT N5 verbs - ALL 161 entries from Minna no Nihongo (including duplicate verbs with different meanings)",
        "totalEntries": len(verbs_list),
        "uniqueVerbs": len(set(f"{v['kanji']}|{v['kana']}" for v in verbs_list)),
...
print(f"📊 Total entries: {len(verbs_list)}")
print(f"   - Unique verbs: {output_json['metadata']['uniqueVerbs']}")
