...
export default function ExtendedKanjiPracticePage() {
  ...
  // No SRS levels for kanji study — just count cards viewed in this session
  // (vs first-time vs already-seen-before).
  const sessionFirstTimeIdsRef = useRef<Set<string>>(new Set());
    ...
    (async () => {
      try {
        ...
        if (urlSort) {
          ...
          // Track which cards in this session are first-time vs revisits
          // for the end-of-session summary.
          sessionFirstTimeIdsRef.current = new Set(
            pick.filter((k) => !progressData[k.id] || progressData[k.id].reviewCount === 0).map((k) => k.id),
          );
          ...
          // Re-build session first-time set so summary still works on reload.
          sessionFirstTimeIdsRef.current = new Set(
            restored.filter((k) => !progressData[k.id] || progressData[k.id].reviewCount === 0).map((k) => k.id),
          );
          setShowConfig(false);
  ...
  const buildOrder = (
    ...
    switch (cfg.sortMode) {
      case 'random':
        ...
        break;
      case 'untouched':
        // Cards the user hasn't viewed yet, in lesson order.
        sorted = pool
  ...
  const startPractice = async (saveAsDefault: boolean) => {
    ...
    // Snapshot first-time-this-session card ids for the end-of-session summary.
    sessionFirstTimeIdsRef.current = new Set(
      pick.filter((k) => !progress[k.id] || progress[k.id].reviewCount === 0).map((k) => k.id),
    );
  ...
  const handleNext = useCallback(() => {
    ...
    setIndex((prev) => {
      ...
      if (nextIdx >= order.length && order.length > 0 && !sessionEndedRef.current) {
        ...
        // A card counts as "viewed" if its lastSeen was updated during this
        // session. Simpler: count all cards in `order` that have any progress
        // entry now. Split by whether they were first-time at session start.
        let reviewed = 0
        let firstTime = 0
        let revisited = 0
        for (const k of order) {
          const p = progress[k.id]
          if (!p || p.reviewCount === 0) continue
          reviewed++
          if (sessionFirstTimeIdsRef.current.has(k.id)) firstTime++
          else revisited++
        }
        setSessionSummary({ reviewed, firstTime, revisited });
  ...
  const handleFlip = () => {
    setFlipped((f) => {
      const next = !f;
      // Mark as viewed the moment the answer side is revealed.
      if (next) markViewed();
      return next;
    });
  };

  // Mark this kanji as "viewed". No SRS levels — kanji content is too compound
  // for atomic recall ratings. Stored shape stays the same (so existing user
  // data isn't invalidated); we just always write level=0 and rely on
  // reviewCount/lastSeen to drive coverage stats on the dashboard.
  const markViewed = () => {
    if (!current) return;
              ...
              <div className="space-y-2">
                {(
                  [
                    ...
                    ['untouched', 'Not yet viewed', 'Cards you haven\'t opened yet, in lesson order'],
    ...
    <div className="container max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                You viewed {sessionSummary.reviewed} of {order.length} cards.
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg border p-3 text-center">
                  <div className="text-2xl font-bold tabular-nums text-primary">{sessionSummary.firstTime}</div>
                  <div className="text-[10px] text-muted-foreground">New</div>
                </div>
                <div className="rounded-lg border p-3 text-center">
                  <div className="text-2xl font-bold tabular-nums">{sessionSummary.revisited}</div>
                  <div className="text-[10px] text-muted-foreground">Revisited</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Button
      ...
      <div className="relative">
        ...
        >
          {hasBeenRated && (
            <div
              className="absolute top-2 right-2 flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50/80 dark:bg-emerald-950/40 px-2 py-1 rounded-full z-10"
              onClick={(e) => e.stopPropagation()}
              title={`Viewed ${currentProgress?.reviewCount}× · last seen ${new Date(currentProgress?.lastSeen ?? Date.now()).toLocaleDateString()}`}
            >
        ...
        <div className="text-center mt-4">
          <Link href={`/study/kanji/${encodeURIComponent(current.id)}`}>
